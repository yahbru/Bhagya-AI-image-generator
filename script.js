...
const styleSelect = document.getElementById("style-select"); // replaced model-select
...
const generateImages = async (
  ...
  const imagePromises = Array.from({ length: imageCount }, async (_, i) => {
    try {
      const styledPrompt = `${promptText}, ${selectedStyle} style`;
      const encodedPrompt = encodeURIComponent(styledPrompt);
      const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=${width}&height=${height}&nologo=true&seed=${
        Date.now() + i
      }`;
...
const createImageCards = (
  ...
  generateImages(selectedStyle, imageCount, aspectRatio, promptText);
};

// Handle form submission
const handleFormSubmit = (e) => {
  e.preventDefault();
  const selectedStyle = styleSelect.value || "realistic";
  const imageCount = parseInt(countSelect.value) || 1;
  const aspectRatio = ratioSelect.value || "1/1";
  const promptText = promptInput.value.trim();
  createImageCards(selectedStyle, imageCount, aspectRatio, promptText);
};
